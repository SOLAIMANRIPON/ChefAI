...
/** Render free tier may sleep; wake before IAP grant reduces "Network request failed". */
async function wakeBillingBackend(apiBaseUrl: string): Promise<void> {
  ...
  try {
    ...
    // Non-fatal — grant retry may still succeed.
  } finally {
    clearTimeout(timer);
  }
}

async function postGrantCredits(
  apiBaseUrl: string,
  body: { productId: string; purchaseToken: string; packageName: string },
  installId: string
): Promise<{ res: Response; payload: { message?: string; billing?: RecipeBillingSnapshot } }> {
  const base = apiBaseUrl.replace(/\/$/, '');
  const controller = new AbortController();
  ...
  try {
    const res = await fetch(`${base}/api/v1/billing/google-play/grant-credits`, {
      ...
      headers: {
        ...
        [CHEFAI_INSTALL_HEADER]: installId,
      },
...
async function grantCreditsFromPurchase(
  ...
  for (let attempt = 0; attempt < 4; attempt++) {
    if (attempt > 0) await sleep(attempt === 1 ? 2000 : 3000);
    try {
      const { res, payload } = await postGrantCredits(params.apiBaseUrl, grantBody, params.installId);
      if (!res.ok) {
  ...
  throw new Error(
    `Could not reach the ChefAI server (${lastError}). Your Play payment may have succeeded — wait 30 seconds, then tap Buy credits again to retry.`
  );
}
...
export async function purchaseCreditsWithGooglePlay(params: {
  ...
  await wakeBillingBackend(params.apiBaseUrl);
